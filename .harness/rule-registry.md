...
| R48 | 字典物理删语义 + 缓存失效键 | 配置数据（字典类型/数据）删除走物理删（@Delete），因 dict_type/dict_value 有 DB 唯一约束、逻辑删残留阻止同名重建 | Mapper @Delete 注解（SysDictTypeMapper/SysDictDataMapper） | ✅ 已生效 |
| R49 | 五险一金智能核算（批次A）：规则表 hr_social_rule（company+social_type+effective_month 唯一，上下限/比例/生效月 CHECK）+ 明细表 hr_social_detail（company+employee+year+month+type 唯一，金额≥0 CHECK）+ employee.social_base 申报基数；核算语义：基数=clamp(申报基数或基本工资, 下限, 上限)（低于下限按下限、高于上限按上限），个人/单位金额=基数×比例（工伤/生育个人 0）；工资核算自动接入（hasRules 前置检查，无规则宽容置 0 不阻塞——避免事务 rollback-only，独立核算接口显式报错）；规则幂等 upsert、物理删（逻辑删与 uq 冲突）；14 硬约束 17（禁魔法值：险种/状态枚举） | V20 迁移 + SocialInsuranceService/Controller（/hr/social 5 接口 @PreAuthorize+@OperLog）+ SalaryService.calculate 自动扣缴 + 前端社保公积金页（规则 tab/明细 tab/一键核算）+ 1404 菜单按钮 | SocialInsuranceServiceTest 10 例（比例/上下限 clamp/工伤个人 0/六险种/幂等/物理删/无规则拦截/工资接入）+ API 实测（H001 申报 10000→社保 1050/公积金 500；H002 下限兜底 4800→504/100） | ✅ 生效 |
| R48b | ArchUnit service_naming 只约束顶层类 | service 包内业务服务类必须 *Service 后缀，内部类/匿名类不受命名规则约束 | ArchitectureTest.service_naming `.and().areTopLevelClasses()` | ✅ 已生效 |
