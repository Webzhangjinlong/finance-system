...
| L48 | AOP 审计切面跨模块实施四坑：①注解放 finance-system → workflow/contract 模块编译失败（模块不依赖 system）→ 注解必须放 finance-common（公共模块），切面留 system（依赖 system Mapper）；②脱敏递归只处理 Map/List → POJO 字段（ResetPwdDTO.password）漏网 → 先 valueToTree 转 JsonNode 再递归脱敏；③ArchUnit 分层未考虑横切（切面直调 Mapper 违反 "Mapper 仅 Service 访问"）→ 分层规则显式增加 Aspect 层并授权；④python 多步替换脚本中途 assert 失败 → 后续修改仅在内存未落盘（菜单项丢失）→ 每步替换后独立验证落盘 | AOP 注解/切面的模块归属（公共 vs 宿主）、脱敏针对 POJO 需 JsonNode 化、ArchUnit 分层需为横切预留层、批量脚本需分步验证 | 注解移 finance-common（全库替换 import 12 文件）+ OperLogAspect 用 valueToTree 脱敏 + ArchitectureTest 加 Aspect 层（Mapper 允许 Service/Aspect 访问）+ 前端补菜单后 git grep 复核 | ✅ 已固化（PR#30） |
| L50 | F9 试算平衡 CREDIT 方向科目期末归位逻辑写反（net<0 贷方净额时误记借方余额）→ 4001 实收资本期末贷 0/借 10万、balanced=false；另 AnalysisServiceTest 造 2026-08 凭证报"会计期间不存在"（test 库仅种子 2026-09 期间）；L48 复现：python 多步 patch 脚本中途 assert 缩进失败 → 整脚本未执行（Service 修复未落盘） | 归位分支把"net 符号→借贷栏"对应写反（CREDIT 科目 net<0 应记贷方）；测试数据未预建期间；批量脚本编译失败整体不执行 | 归位逻辑统一为：DEBIT 科目正净额→借、负→贷；CREDIT 科目正→借、负→贷（净额=借-贷 统一口径）；测试 BeforeEach 幂等补插所需期间（INSERT WHERE NOT EXISTS）；python 脚本每步替换后独立验证落盘（L48 流程重申） | ✅ 已固化（PR#34） |
...
| L53 | 批次A 五险一金 事务 rollback-only 深坑：SalaryService.calculate(@Transactional) 内 try/catch 包裹 SocialInsuranceService.calculate(@Transactional REQUIRED 同事务)，内部抛 BusinessException（无规则）→ 事务标记 rollback-only，外层 catch 吞掉后继续执行，提交时报 UnexpectedRollbackException → HrServiceTest 7 例 ERROR | 跨 Service 事务调用对内部异常 try/catch 无效：Spring 事务传播（REQUIRED 同事务）下内部异常必标记 rollback-only，与外层是否捕获无关 | 改为调用前 hasRules() 预检查（无规则不调用，宽容置 0），避免异常路径；教训：跨 Service 事务调用禁止 try/catch 内部业务异常，用前置状态检查 + 独立核算接口显式报错 | ✅ 已固化（hasRules 预检查 + PR#40） |
| L54 | 前端 api 导出名与页面 import 不一致：social/index.vue import { employeePage } 但 api/index.js 实际导出 pageEmployee → vite build 报 "not exported"；本地 build 因系统内存不足（free<1GB）反复失败掩盖了真实错误，CI frontend-ci 内存充足一次抓到 | 本地环境故障会掩盖真实代码错误；CI 是前端构建的最终硬证据 | 修 import 为 pageEmployee；教训：本地 build 受环境限制失败时，不得据此宣称代码正确，必须等 CI 结果；import 名与 api 导出名逐一核对 | ✅ 已固化（PR#40 修复 commit + CI 双绿） |
